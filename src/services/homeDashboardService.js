...
async function setsCompletedPercentFromSessionId(sessionId) {
  ...
  const daeDocs = await WorkoutDayExercise.find({ _id: { $in: daeIds } }).select('prescribedSets');
  const totalPrescribed = daeDocs.reduce((s, d) => s + (d.prescribedSets || 0), 0);
...
async function todayNutrition(userId, dateKey) {
  ...
  }).select('calories protein carbs fats adjustedCalories adjustedProtein adjustedCarbs adjustedFats');
...
export async function buildDashboard(userId, dateKey, timeZone) {
  ...
  if (isCalendar) {
    const { markMissedCalendarDaysForUser } = await import('./workoutOccurrenceService.js');
...
async function buildCalendarDashboard(userId, dateKey, timeZone, plan) {
  ...
  const allDays = await WorkoutDay.find({ workoutPlan: plan._id, isRestDay: false })
  ...
  return {
    hasPlan: true,
    plan: { _id: plan._id, name: plan.name, generationType: plan.generationType, planShape: plan.planShape },
...
async function buildTemplateDashboard(userId, dateKey, timeZone, plan) {
  ...
  return {
    hasPlan: true,
    plan: { _id: plan._id, name: plan.name, generationType: plan.generationType, planShape: plan.planShape || 'template' },
    streak: { current: streak },
    todayCard,
    weekSchedule,
    todayNutrition: nutrition,
...
async function assertDayOnPlan(dayId, planId) {
  const day = await WorkoutDay.findOne({ _id: dayId, workoutPlan: planId }).select('_id name');
  if (!day) {
    throw Object.assign(new Error('WorkoutDay not found on your active plan.'), { status: 404 });
  }
  return day;
}
...
export async function listReplacements(userId) {
  const plan = await getActivePlan(userId);
  if (!plan) throw Object.assign(new Error('No active workout plan.'), { status: 404 });
...
export async function createReplacement(userId, { originalWorkoutDayId, replacementWorkoutDayId }) {
  if (!originalWorkoutDayId || !replacementWorkoutDayId) {
    throw Object.assign(
      new Error('originalWorkoutDayId and replacementWorkoutDayId are required.'),
      { status: 400 },
    );
  }
  if (String(originalWorkoutDayId) === String(replacementWorkoutDayId)) {
    throw Object.assign(new Error('A workout day cannot replace itself.'), { status: 400 });
  }
  ...
  const replacementDay = await WorkoutDay.findById(replacementWorkoutDayId).select('isRestDay');
  if (replacementDay && replacementDay.isRestDay) {
    throw Object.assign(new Error('Cannot replace a workout with a rest day.'), { status: 400 });
  }

  const replacement = await WorkoutDayReplacement.findOneAndUpdate(
    { user: userId, workoutPlan: plan._id, originalWorkoutDay: originalWorkoutDayId },
    {
      $set: {
        user: userId,
        workoutPlan: plan._id,
